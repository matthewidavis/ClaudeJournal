...
def render_site(db_path: Path, out_dir: Path, claude_home: Path) -> dict:
    ...
    from claudejournal.entity_synthesis import load_entity_synthesis
    ...
    stats["entity_synthesis_pages"] = 0
    for _ent_row in _qualifying:
        ...
        # Load LLM synthesis prose if available (generated by entity_synthesis stage).
        _synthesis_prose = load_entity_synthesis(conn, _cname)
        if _synthesis_prose:
            stats["entity_synthesis_pages"] += 1
        _profile_html = render_entity_profile_page(
            _ent_data,
            anchor_base="../",
            synthesis_prose=_synthesis_prose,
            known_docs=known_docs_all,
            known_topics=known_topics_all,
        )
